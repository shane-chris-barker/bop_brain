...
patch_config = "events.factories.publisher_factory.get_config"

@patch(patch_config)
@patch("events.factories.publisher_factory.MqttPublisher")
def test_it_returns_mqtt_publisher(mock_mqtt_publisher_class, mock_get_config):
    mock_get_config.return_value = MagicMock(
        COMM_TYPE='mqtt',
        MQTT_HOST='localhost',
        MQTT_PORT=1883,
        OUTPUT_COMM_NAME='test_topic'
    )
...
@patch(patch_config)
@patch("events.factories.publisher_factory.AmqpPublisher")
def test_it_returns_amqp_publisher(mock_amqp_publisher_class, mock_get_config):
    mock_get_config.return_value = MagicMock(
        COMM_TYPE='amqp',
        AMQP_HOST='localhost',
        AMQP_PORT=5672,
        OUTPUT_COMM_NAME='test_queue'
    )
